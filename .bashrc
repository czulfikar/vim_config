...
function __makePS1() {
    ...
    if [ -n "${SSH_CONNECTION}" ]; then
        PS1+="\[${Blue}\]@"
        PS1+="\[${UBlue}${HOST_COLOR}\]\h\[${Color_Off}\]" # host displayed only if ssh connection
    fi
...
if [ "$color_prompt" = yes ]; then
    ...
    PS2="\[${Purple}\]>\[${Color_Off}\] " # continuation prompt
else
    PS1='${debian_chroot:+($debian_chroot)}\u@\h:\w\$ '
fi

### SECTION below replaced with above color schema settings
